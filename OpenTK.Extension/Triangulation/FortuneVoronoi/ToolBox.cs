...
namespace FortuneVoronoi
{
	public abstract class MathTools
	{
		...
		{
			return Convert.ToSingle(Math.Sqrt((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1)));
		}
...
//				throw new Exception("Shuffle only defined on one-dimensional arrays!");
		...
		public static float[] HSBtoRGB(int hue, int saturation, int brightness, float[] OldCol)
		{
			...
			int i = (int)Math.Floor(hue/60f),j;
			float[] C;
			...
			float min = 127f * (240 - saturation)/240f;
			float max = 255f - 127 * (240 - saturation)/240f;
			if(brightness>120)
			{
				min = min + (255f-min)*(brightness-120)/120f;
				max = max + (255f-max)*(brightness-120)/120f;
			}
			if(brightness<120)
			{
				min = min * brightness / 120f;
		...
		public static float GetAngle(float x, float y)
		{
			...
			{
				if(y>0)
					return Convert.ToSingle(Math.PI/2f);
				if(y==0)
					return 0;
				if(y<0)
					return Convert.ToSingle(Math.PI*3f/2f);
			}
			float atan = Convert.ToSingle(Math.Atan(y/x));
			if(x>0 && y>=0)
				return atan;
			if(x>0 && y<0)
				return Convert.ToSingle(2*Math.PI+atan);
			return Convert.ToSingle(Math.PI+atan);
		}
		...
		public static bool intersect(Vector3 P11, Vector3 P12, Vector3 P21, Vector3 P22)
		{
			return ccw(P11, P12, P21, true)*ccw(P11, P12, P22, true) <= 0
			    && ccw(P21, P22, P11, true)*ccw(P21, P22, P12, true) <= 0;
		}

		public static Vector3 IntersectionVector3(Vector3 P11, Vector3 P12, Vector3 P21, Vector3 P22)
		{
			...
			if(a != 0f)
			{
				return new Vector3((float)(Kx+a*Lx),(float)(Ky+a*Ly), 0f);
			}
            //if (!float.IsNaN(b))
            if (b != 0f)
			{
				return new Vector3((float)(Mx+b*Nx),(float)(My+b*Ny), 0f);
			}
			throw new Exception("Error in IntersectionVector3");
		}
    }
}